...
### 🚨 트러블슈팅: "Wrong Directory Deployed!" 화면이 뜬다면?
만약 배포 후 빨간색 배경나 "Wrong Directory Deployed!"라는 화면이 뜬다면 **Root Directory** 설정이 잘못된 것입니다.

1. Vercel 대시보드 > 해당 프로젝트 > **Settings** > **General** 로 이동합니다.
2. **Root Directory** 항목을 찾습니다.
3. `Edit`을 누르고 **`web`** 폴더를 선택한 뒤 저장합니다.
4. **Deployments** 탭으로 이동하여, 가장 최근 배포의 점 3개 메뉴(...)를 누르고 **Redeploy**를 클릭합니다. (설정 변경은 재배포해야 적용됩니다.)
